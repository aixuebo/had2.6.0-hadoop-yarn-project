...
public abstract class NMContainerStatus {
  ...
  public static NMContainerStatus newInstance(ContainerId containerId,
    ...
    status.setContainerId(containerId);//容器ID
    status.setContainerState(containerState);//当前状态
    status.setAllocatedResource(allocatedResource);//该容器分配的资源
    status.setDiagnostics(diagnostics);//信息
    status.setContainerExitStatus(containerExitStatus);//退出状态
    status.setPriority(priority);//优先级
    status.setCreationTime(creationTime);//创建时间
    return status;
  }
