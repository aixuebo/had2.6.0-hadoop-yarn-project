...
public abstract class AllocateRequest {
  ...
  /**
   * 
   * @param responseID 上一次RMContainerRequestor类收到的回复ID,responseID
   * @param appProgress 该job目前执行的进度
   * @param resourceAsk 要尚需要被请求的资源集合
   * @param containersToBeReleased 已经完成的容器资源集合
   * @param resourceBlacklistRequest 针对该job,需要删除一些黑名单host信息,以及增加一些黑名单host信息
