...
public class RegistryCli extends Configured implements Tool {
  ...
  static final String LS_USAGE = "ls pathName";//查看
  static final String RESOLVE_USAGE = "resolve pathName";//解析path下的字节数组为ServiceRecord对象
  static final String BIND_USAGE = "bind -inet -p portNumber -h hostName -api apiName pathName"
      + "\n" + "bind -webui uriString -api apiName  pathName" + "\n"
      + "bind -rest uriString -api apiName  pathName";
  static final String MKNODE_USAGE = "mknode directoryName";//创建路径
  static final String RM_USAGE = "rm pathName";//rm pathName -r
  ...
  public int run(String[] args) throws Exception {
    ...
    registry = RegistryOperationsFactory.createInstance(new YarnConfiguration(getConf()));
    registry.start();
    if (args.length > 0) {
      if (args[0].equals("ls")) {
        return ls(args);
    ...
    try {
      ...
      try {
        ...
        for (Endpoint endpoint : record.external) {
          sysout.println(" Endpoint(ProtocolType=" + endpoint.protocolType + ", Api="
              + endpoint.api + ");" + " Addresses(AddressType=" + endpoint.addressType + ") are: ");

          for (Map<String, String> address : endpoint.addresses) {
            sysout.println("  [ ");
            for (Map.Entry<String, String> entry : address.entrySet()) {
              sysout.println("    " + entry.getKey() + ": \"" + entry.getValue() + "\"");
            }
            sysout.println("  ]");
          }
          sysout.println();
  ...
  public int bind(String[] args) {
    Option rest =
        OptionBuilder.withArgName("rest").hasArg().withDescription("rest Option").create("rest");
    Option webui =
        OptionBuilder.withArgName("webui").hasArg().withDescription("webui Option").create("webui");
    Option inet = OptionBuilder.withArgName("inet").withDescription("inet Option").create("inet");
    Option port =
        OptionBuilder.withArgName("port").hasArg().withDescription("port to listen on [9999]")
            .create("p");
    Option host =
        OptionBuilder.withArgName("host").hasArg().withDescription("host name").create("h");
    Option apiOpt = OptionBuilder.withArgName("api").hasArg().withDescription("api").create("api");
    //bind -inet -p portNumber -h hostName  -api apiName  pathName
    ...
    if (args[1].equals("-inet")) {
      ...
      if (line.hasOption("inet") && line.hasOption("p") && line.hasOption("h")
          && line.hasOption("api")) {
        portNum = Integer.parseInt(line.getOptionValue("p"));
        hostName = line.getOptionValue("h");
        api = line.getOptionValue("api");
        sr.addExternalEndpoint(inetAddrEndpoint(api, ProtocolTypes.PROTOCOL_HADOOP_IPC, hostName,
            portNum));
  ...
  /**
   * Given an exception and a possibly empty argument list, generate a diagnostics string for use in
   * error messages
   * 
