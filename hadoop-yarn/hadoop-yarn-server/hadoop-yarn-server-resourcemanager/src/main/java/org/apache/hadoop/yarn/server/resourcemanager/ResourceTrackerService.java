...
public class ResourceTrackerService extends AbstractService implements
  ...
  public RegisterNodeManagerResponse registerNodeManager(RegisterNodeManagerRequest request) throws YarnException,IOException {
    ...
    // Check if this node has minimum allocations 如果该节点持有的资源不足最小配置,则返回信息
  ...
  public NodeHeartbeatResponse nodeHeartbeat(NodeHeartbeatRequest request)
    ...
    // Send ping 该节点有心跳,则相当于该节点有ping
    this.nmLivelinessMonitor.receivedPing(nodeId);

    // 2. Check if it's a valid (i.e. not excluded) node 白名单没有过,则返回shutdown
    ...
    //如果节点的+1等于上次节点的信息,说明节点重复发送心跳了,则返回上一个节点信息即可
    if (remoteNodeStatus.getResponseId() + 1 == lastNodeHeartbeatResponse.getResponseId()) {
      ...
      //说明节点落后ResourceManager太远了,重新同步吧
      String message =
