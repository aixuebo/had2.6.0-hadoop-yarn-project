...
public class ContainerManagerImpl extends CompositeService implements
  ...
  /**
   * 等候一直到没有容器为止,如果循环100次还有容器,则也退出循环
   */
  private void waitForRecoveredContainers() throws InterruptedException {
    final int sleepMsec = 100;//每次睡眠时间
    int waitIterations = 100;//最多循环多少次
    ...
    while (--waitIterations >= 0) {
      ...
      if (newContainers.isEmpty()) {//发现没有空的新容器,则退出循环
        break;
      }
