...
public class LogCLIHelpers implements Configurable {
  ...
  /**
   * 打印一个容器的日志内容
   * @param containerIdStr 容器路径
   * @param reader 日志的反序列化工具
   * @param out 输入目录
   * @param logUploadedTime 日志的最后更新时间
   */
  @Private
  public int dumpAContainerLogs(String containerIdStr,
    ...
    while (true) {
      try {
        //向输出流中写入数据
        LogReader.readAContainerLogsForALogType(valueStream, out,logUploadedTime);
        foundContainerLogs = true;
