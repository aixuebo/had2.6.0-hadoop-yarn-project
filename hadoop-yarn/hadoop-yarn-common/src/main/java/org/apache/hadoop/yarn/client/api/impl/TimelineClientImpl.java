...
/**
 * 历史记录客户端
 * 以rest方式发送htt的post请求发送到制定url中记录日志
 * 具体服务器端如何记录日志,则参照 hadoop-yarn-server-applicationhistoryservice项目
 */
@Private
@Unstable
public class TimelineClientImpl extends TimelineClient {
  ...
  static {
    opts = new Options();
    opts.addOption("put", true, "Put the timeline entities/domain in a JSON file");//制定json文件
    opts.getOption("put").setArgName("Path to the JSON file");
    opts.addOption(ENTITY_DATA_TYPE, false, "Specify the JSON file contains the entities");//告诉我们json文件是entity类型的
    opts.addOption(DOMAIN_DATA_TYPE, false, "Specify the JSON file contains the domain");//告诉我们json文件是domain类型的
  ...
  /**
   * 用rest方式发送post请求,求求on个接收数据是json数据,将object信息发送过去,返回是ClientResponse对象
   */
  @Private
  @VisibleForTesting
