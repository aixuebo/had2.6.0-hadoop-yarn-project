...
public class ProportionalCapacityPreemptionPolicy implements SchedulingEditPolicy {
  ...
  private TempQueue cloneQueues(CSQueue root, Resource clusterResources) {
    ...
    synchronized (root) {
      ...
      /**
       * 乘法计算当前已经使用的资源量、真实的分配资源、分配的最大资源、
       */
  ...
  static class TempQueue {
    final String queueName;//队列名称
    final Resource current;//当前队列已经使用的资源
    final Resource pending;//当前队列等待的资源,如果是非叶子节点,则该值为所有子叶子节点之和的等待资源值
    final Resource guaranteed;//当前队列应该有的资源
    final Resource maxCapacity;//当前毒烈最大允许分配的资源
    ...
    final ArrayList<TempQueue> children;//非叶子节点时候有该值
    LeafQueue leafQueue;//叶子节点
