...
public class WindowsResourceCalculatorPlugin extends ResourceCalculatorPlugin {
  ...
  private final int refreshIntervalMs = 1000;//执行周期
  ...
  void refreshIfNeeded() {
    ...
    if (now - lastRefreshTime > refreshIntervalMs) {//大于周期就刷新一次
      long refreshInterval = now - lastRefreshTime;//时间间隔
      lastRefreshTime = now;
      long lastCumCpuTimeMs = cumulativeCpuTimeMs;//上次cpu时间
      reset();
