...
class LocalResourcesTrackerImpl implements LocalResourcesTracker {
  ...
  private LocalizedResourceProto buildLocalizedResourceProto(LocalizedResource rsrc) {
...
/**
 ...
 // current synchronization guaranteed by crude RLS event for cleanup
   ...
   *        localize the resource. 待下载的资源
   * @param {@link Path} local directory path 下载到本地的目录
   * 为res参数设置本地存储路径
   */
  @Override
  public Path getPathForLocalization(LocalResourceRequest req, Path localDirPath) {
    ...
    rPath = new Path(rPath,Long.toString(uniqueNumberGenerator.incrementAndGet()));
